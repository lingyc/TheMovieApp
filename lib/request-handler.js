...
var request = {requestor: req.mySession.user, requestee: req.body.requestee,requestTyp:'watch'};
...
exports.listRequests = function(req, response) {
  ...
  con.query('Select * FROM allrequests WHERE requestee= ?', request, function(err,res){
  if(err) throw err;
  console.log(res)
  response.send(res);
});
...
con.query('DELETE FROM allrequests WHERE requestor = '+'"'+ requestor+'"'+' AND requestee='+'"'+ requestee+'"', function(err, res) {
        if (err) throw err;
        console.log('Last insert ID:', res.insertId);
...
exports.getFriends = function(req, res) {
  ...
  con.query('SELECT id FROM users WHERE username = ?', id, function(err, resp) {
   ...
   console.log('should be all ratings of mineeee',resp);
   con.query('SELECT * FROM relations WHERE user1id = ?', userid, function(err, resp) {
console.log('this should be your friends',resp)
      ...
      con.query('SELECT username FROM users WHERE id = ?', a, function(err, respo) {
      	console.log('identifying users resp',respo)
      	keyId[a]=respo[0].username;
        con.query('SELECT * FROM ratings WHERE userid ='+'"'+a+'"', function(err, resp) {
        console.log('these should be all of friends ratings',resp)
people.push(resp.map(function(a){return [a.userid,a.movieid,a.score];}));
