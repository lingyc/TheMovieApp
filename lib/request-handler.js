...
const sum= final.reduce(function(a,b){return a+b},0);
...
exports.sendWatchRequest = (req, response)=> {
	...
	Promise.each(requestees, requestee =>{
      ...
      message: req.body.message,
		...
		con.query('INSERT INTO allrequests SET ?', request, (err,res)=>{
		  if(err) throw err;
...
exports.sendRequest = (req, response)=> {
  ...
  if (req.mySession.user === req.body.name) {
    ...
    con.query('SELECT requestee,response FROM allrequests WHERE  requestor = ? AND requestTyp =' + '"' + 'friend' + '"', request['requestor'], function(err, res) {
      ...
      var pplReqd = res.filter(a=> (
         a.response === null
      ))
      var requestees = pplReqd.map(a=> (
         a.requestee
      ))
      ...
      con.query('INSERT INTO allrequests SET ?', request, (err, resp)=> {
        if (err) throw err;
...
exports.signinUser = (req, res)=> {
	...
	new User({ username: req.body.name }).fetch().then(found=>{
		if (found){
			new User({ username: req.body.name, password:req.body.password}).fetch().then(found=>{
				if (found){
