...
exports.getRecentRelease = function(req, res) {
  ...
  var data = '';
	...
	.on('end', function(){
		...
		//formating the obj so it will work with our getMultipleMovieRatings handler
    var recentMoviesParsed = recentMovies.results.map(recentMovie => {
      return movieObj = {
        ...
        description: recentMovie.overview.slice(0, 255),
        imdbRating: recentMovie.vote_average
      }
    });

    req.body.movies = recentMoviesParsed;
